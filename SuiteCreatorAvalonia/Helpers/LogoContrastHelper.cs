...
namespace SuiteCreatorAvalonia.Helpers
{
    ...
    // drawing a rectangular badge behind it. Operates on already-decoded bitmaps (rather than a
    // file path) since logos here are typically loaded from embedded assets or persisted base64.
    internal static class LogoContrastHelper
    {
        ...
        public static bool NeedsOutline(AvaloniaBitmap? logo, Color backgroundColour)
        {
            Color? logoColour = ComputeAverageColour(logo);
        ...
        public static AvaloniaBitmap? CreateLogoWithOutline(AvaloniaBitmap? logo, Color outlineColour)
        {
            if (logo is null)
                return null;
            ...
            try
            {
                using DrawingBitmap source = ToDrawingBitmap(logo);
            ...
            {
                AppLog.Warning($"Failed to generate logo outline: {ex.Message}", nameof(LogoContrastHelper));
                return null;
            }
        }

        private static DrawingBitmap ToDrawingBitmap(AvaloniaBitmap logo)
        {
            using MemoryStream ms = new MemoryStream();
            logo.Save(ms);
            ms.Seek(0, SeekOrigin.Begin);
            return new DrawingBitmap(ms);
        }
        ...
        private static Color? ComputeAverageColour(AvaloniaBitmap? logo)
        {
            if (logo is null)
                return null;
            ...
            try
            {
                using DrawingBitmap drawingBitmap = ToDrawingBitmap(logo);
            ...
            {
                AppLog.Warning($"Failed to compute average logo colour for contrast check: {ex.Message}", nameof(LogoContrastHelper));
                return null;
            }
        }
