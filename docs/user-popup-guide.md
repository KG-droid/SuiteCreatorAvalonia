...
- If the mic is in use, the suite exits immediately (exit code 1602, so your deployment tool records the run as pending rather than being kept waiting) and leaves behind its own short-interval scheduled task that rechecks every minute for up to 4 hours, rather than relying on your deployment tool's own re-evaluation schedule to happen to land between meetings. Once the mic is free, that recheck run continues straight on to show the popup — no separate deployment tool cycle needed. If a meeting runs past the 4-hour cap, the popup shows anyway as a fail-safe.
- It's independent of the **Max Delay Days** allowance — pausing for a meeting never eats into a user's deferral budget.
- It fails safe: if the check can't run for any reason (no active session, an error querying the microphone), the popup shows as normal rather than the suite waiting indefinitely.
