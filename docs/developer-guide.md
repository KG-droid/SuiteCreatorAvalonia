...
2. The stub then **installs or updates** `SuiteExecutor.exe`, `SuiteUserPopup.exe`, and `SuiteProgressPopup.exe` into `%ProgramFiles%\SuiteExecutor`, keeping the newest version, and launches the installed Executor pointing at the extracted `SuiteConfig.scfg`. This install location is **shared across every suite on the machine** — it isn't per-suite. The stub compares its own extracted `SuiteExecutor.exe`'s file version against whatever is already installed there and only overwrites when its own build is newer (or the installed exe is missing entirely), then always launches whichever copy ends up newest. So an old suite deployed a year ago will happily update the shared runtime if it's the newest thing to run, and a newer suite deployed afterwards will update it again — the machine always ends up running the latest Executor code regardless of which suite last triggered the update, and every suite that runs picks up the same shared copy rather than its own bundled one. Because the popup exes (`SuiteUserPopup.exe`, `SuiteProgressPopup.exe`) are versioned independently of `SuiteExecutor.exe`, the stub separately SHA-256-hashes the installed vs. extracted popup exes and force-updates them (bypassing the version check) if they differ — so a popup-only fix ships even when the Executor's own version number hasn't moved.
